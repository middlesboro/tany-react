...
const Checkout = () => {
  ...
  const validateForm = () => {
      ...
      // Customer
      if (!isValidName(customer.firstname)) newErrors.firstname = "Invalid first name (only characters allowed)";
      if (!isValidName(customer.lastname)) newErrors.lastname = "Invalid last name (only characters allowed)";
      if (!isValidEmail(customer.email)) {
          newErrors.email = "Invalid email format";
      } else {
          const typo = checkEmailTypos(customer.email);
          if (typo) newErrors.email = typo;
      }
      if (!isValidSlovakPhone(customer.phone)) newErrors.phone = "Invalid Slovak phone number";

      // Invoice Address (Required)
      if (!invoiceAddress.street) newErrors.invoice_street = "Street is required";
      if (!invoiceAddress.city) newErrors.invoice_city = "City is required";
      if (!isValidSlovakZip(invoiceAddress.zip)) newErrors.invoice_zip = "Invalid ZIP (must be 5 digits)";

      // Delivery Address (Required if different)
      if (differentDeliveryAddress) {
          if (!deliveryAddress.street) newErrors.delivery_street = "Street is required";
          if (!deliveryAddress.city) newErrors.delivery_city = "City is required";
          if (!isValidSlovakZip(deliveryAddress.zip)) newErrors.delivery_zip = "Invalid ZIP (must be 5 digits)";
      }
