...
const FilterParameterValueList = () => {
  const {
    ...
    size, setSize,
    sort, handleSort
  } = usePersistentTableState('admin_filter_parameter_values_list_state', {}, 'name,asc');
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        ...
        <Link to="/filter-parameter-values/new" className="bg-green-500 text-white px-4 py-2 rounded">
          Add New
        </Link>
      </div>
        ...
        <tbody>
          {values.map((val) => (
            <tr key={val.id}>
              ...
              <td className="py-2 px-4 border-b">
                <Link
                  to={`/filter-parameter-values/${val.id}`}
